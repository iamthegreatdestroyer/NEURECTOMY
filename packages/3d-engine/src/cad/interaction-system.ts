    ...
    switch (type) {
      ...
      case SelectionType.SUBTRACTIVE:
        ...
        if (this.primary === object) {
          const nextValue = this.selected.values().next();
          this.primary =
            this.selected.size > 0 && !nextValue.done ? nextValue.value : null;
        }
        break;
    }
...
export class InteractionSystem {
  ...
  private _transformSpace: TransformSpace = TransformSpace.WORLD;
  ...
  getWorldPositionOnPlane(
    plane: THREE.Plane = this.dragPlane
  ): THREE.Vector3 | null {
    this.raycaster.setFromCamera(this.mouse, this.camera);
  ...
  private handleMouseUp(event: MouseEvent): void {
    ...
    const target = this.getObjectUnderCursor();
    const _clickDuration = performance.now() - this.mouseDownTime;
  ...
  private getObjectsInSelectionBox(): THREE.Object3D[] {
    ...
    const minX = Math.min(
      this.selectionBox.startScreen.x,
      this.selectionBox.endScreen.x
    );
    const maxX = Math.max(
      this.selectionBox.startScreen.x,
      this.selectionBox.endScreen.x
    ...
    const minY = Math.min(
      this.selectionBox.startScreen.y,
      this.selectionBox.endScreen.y
    );
    const maxY = Math.max(
      this.selectionBox.startScreen.y,
      this.selectionBox.endScreen.y
  ...
  private createDragEvent(event: MouseEvent): DragEvent {
    ...
    return {
      ...this.createInteractionEvent(
        "drag",
        event,
        this.selectionManager.getPrimary(),
        []
      ),

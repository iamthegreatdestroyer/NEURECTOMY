...
const API_BASE_URL = import.meta.env.VITE_API_URL || "http://localhost:16080";
const ML_API_URL = import.meta.env.VITE_ML_API_URL || "http://localhost:16081";
...
export const coreApi = {
  // Health check
  health: () =>
    request<{ status: string; version: string }>(API_BASE_URL, "/health"),
  ...
  agents: {
    ...
    get: (id: string) => request<unknown>(API_BASE_URL, `/api/v1/agents/${id}`),
  ...
  // Containers
  containers: {
    list: () => request<unknown[]>(API_BASE_URL, "/api/v1/containers"),
    ...
    logs: (id: string, params?: { tail?: number; since?: string }) =>
      request<string>(API_BASE_URL, `/api/v1/containers/${id}/logs`, {
        params,
      }),
  ...
  // Kubernetes
  clusters: {
    list: () => request<unknown[]>(API_BASE_URL, "/api/v1/clusters"),
...
export const mlApi = {
  ...
  // Models
  models: {
    list: () => request<unknown[]>(ML_API_URL, "/api/v1/models"),

    get: (id: string) => request<unknown>(ML_API_URL, `/api/v1/models/${id}`),
